  ...
  return html`
    ...
    <div class="sx-auth-full">
      <video class="sx-auth-bgvideo" aria-hidden="true"
        ...
        <source src="/static/assets/spiderx-background.mp4" type="video/mp4" />
      </video>
...
function App() {
  ...
  const closeSession = useCallback(async () => {
    ...
    // Drop back to / when a session ends, unless we're showing a cockpit OR
    // we're the embed widget. The embed lives at /embed/<slug>; navigating it
    // to "/" clears embedSlug, so the gate falls through to the landing/
    // marketing splash INSIDE the little iframe — the "UI gets distorted after
    // ending a call" the tester reported (#10). Staying put returns the embed
    // to its own orb + "Talk to <agent>" CTA.
    if (location.pathname !== "/" && !revealAgent && !location.pathname.startsWith("/embed/")) goRoute("/");
    refreshAgents();
