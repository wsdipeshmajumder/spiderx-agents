...
4. **Workspace switcher now functional** — Dashboard workspace dropdown (`WorkspaceSelector` component) had a TODO comment instead of an implementation — clicking a workspace did nothing. Root cause: POST `/api/me/org` endpoint was never written, and frontend had no onClick handler. Fix: (a) Added `switch_org()` endpoint (backend/app.py:226–248) that validates user org membership, updates `users.primary_org_id`, and returns the new org; (b) Replaced TODO comment with async onClick handler (frontend/app.js:5936–5951) that calls the endpoint with org_id, reloads page on success, and logs errors. Evidence: code change in both `backend/app.py` and `frontend/app.js`; endpoint mirrors the canonical pattern of other `/api/me/*` routes (membership check, UPDATE, return resource).
...
## Round 11 (the confirmed external root cause, and the code-level guarantee)

Operator asked to find the root cause of Round 10's remaining ~10s reconnect churn, rather than accept the partial fix. Investigated via web search + the Google AI developer forum + the `googleapis/python-genai` GitHub tracker instead of further guessing.

**Confirmed, Google-acknowledged, still-open bug**: [`gemini-3.1-flash-live-preview` ignores `automatic_activity_detection.silence_duration_ms`](https://github.com/googleapis/python-genai/issues/2580) — our exact production model. Filed 2026-06-10, reproduced by Google engineering ("We have successfully reproduced the issue... we will file a bug internally"), still OPEN at `priority: p2` with "fix may not be included in next release" as of this session, multiple independent developers confirming the same symptom a month later with no fix. Our code explicitly configures `silence_duration_ms=2000` (2s of tolerance, LOW sensitivity) specifically so Gemini waits before treating a pause as end-of-speech — this bug means that config is silently ignored, and the server ends turns/streams far more aggressively than requested. That's the mechanism behind the ~5-10s reconnect cycling seen on every incident so far (all of which involved a deliberately silent caller — exactly the condition this bug's timing sensitivity would expose fastest).

Because this is an external, Google-side bug with no ETA, a prompt-wording fix (build 418) can reduce but not guarantee correct behavior — evidenced directly: a second live call (338) still showed one phantom turn (a Hindi restatement of the recording disclosure) despite the "stay COMPLETELY SILENT" instruction being sent on that exact reconnect. LLM instruction-following was never going to be 100% reliable against a model with a confirmed timing bug feeding it more reconnects than intended.

| Item | Acceptance criterion | Verdict | Tier | Build | Notes |
|---|---|---|---|---|---|
| U41 | Whatever the model says immediately after a reconnect never reaches the caller or the persisted transcript unless real caller speech has occurred since | PASS | Unit | 419 | New `_SessionState.suppress_until_real_input`, set `True` on every reconnect kickoff (all three branches — resume_handle, memory-replay, no-handle — not just the one build 418 touched) and cleared ONLY by a genuine `sc.input_transcription` arriving. While set, `_pump_gemini_to_client` discards the model's audio (never sent to the client/recording/Fish) AND its output_transcription (never fed to conversation memory or the client transcript stream) — logged for diagnosis (`suppressed post-reconnect phantom turn: ...`) but otherwise invisible. This is a code-level guarantee, not an instruction the model can choose to ignore — the deliberate answer to a bug we can't fix upstream. 3 new tests (`TestPostReconnectSuppression`) cover: a phantom turn produces zero audio and never enters memory; real caller speech both lifts suppression AND lets that turn's response play; default (non-suppressed) behavior is unchanged |
| — | Why not just fix the reconnect frequency itself? | Not fixable on our side | — | — | The excessive reconnect rate IS the Google-side bug (`silence_duration_ms` being ignored) — there's no config on our end that currently works around it (confirmed by the forum thread: a context-window-compression workaround was found for a DIFFERENT, audio+video-specific 1011-error bug, not this one). Switching off `gemini-3.1-flash-live-preview` entirely was considered and rejected here — the code's own top-of-file comment already documents that the alternative model family ignores `realtime_input_config` and `language_code` entirely, which would silently undo the VAD tuning and locale handling this bridge depends on. Build 419's suppression guarantee is the correct scope: make the SYMPTOM harmless rather than chase an upstream bug with no ETA |
