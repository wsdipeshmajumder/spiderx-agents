...
| 12 | Bot holds context; doesn't repeat the caller's last question | PARTIAL (hardened; needs live re-test) | Code | (prompt+bridge) | Root cause found: on a Gemini-Live **session reconnect** (fires every few min mid-call) the `<call_resumed>` marker was **never actually sent** — WsStart (which sends `<call_start>`) only fires on the first carrier stream — so the resumed session replayed Gemini's restored context and sometimes **re-answered the caller's last question**. Fix: (a) `_bridge` now sends `<call_resumed>` at reconnected-session start (best-effort, can't break the call); (b) prompt hardened — "answer directly, never read the question back / re-answer what you already handled", and the `<call_resumed>` handling rewritten to "continue seamlessly, no repeat/re-ask/re-greet". **Needs a live call to confirm** (no mic in automation) |
