...
def _agent_chat_system_prompt(agent: dict[str, Any]) -> str:
    ...
    parts.append(
        ...
        "• ORDER / TRANSACTION STATUS: you have NO live access to the order system, so you cannot "
        "look up any order. Do this GRACEFULLY: as soon as the visitor asks about an order, "
        "delivery, or tracking, do NOT ask for their order number and do NOT say you'll 'check' it "
        "(that strings them along). In ONE reply, tell them you can't look up orders here and point "
        "them to where they CAN track it themselves — the tracking link in their order-confirmation / "
        "shipping email, or their account page on the store, plus the contact page for help. Use the "
        "exact order-tracking URLs from your knowledge if present. NEVER invent or guess an order "
        "status, tracking number, or delivery date, even if they give you an order number.\n"
        ...
        "options?\" unless that exact thing is in your knowledge and you can produce it in your very next "
        "message. If you can't deliver it, don't offer it: answer with what you DO have, state plainly you "
        "don't have that specific document/detail on hand, and offer to connect them to the team "
        "(`request_human_handoff`) to get it. Offering something and then not producing it is worse than "
